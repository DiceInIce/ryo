...
// Дефолтный URL API - используем локальный бэкенд, если переменная не задана
const defaultApiUrl = import.meta.env.VITE_APP_API_URL || 'http://localhost:8000'

remoteConfig.defaultConfig = {
  api_endpoints: JSON.stringify([
    {
      url: defaultApiUrl,
      description: 'Local Backend API'
    }
  ]),
  load_script: false
}
...
function parseApiEndpoints(configValue) {
  try {
    ...
    const fallbackUrl = import.meta.env.VITE_APP_API_URL || 'http://localhost:8000'
    return [
      {
        url: fallbackUrl,
        description: 'Fallback API (Local Backend)'
      }
    ]
  }
}
...
async function getCurrentApiUrl() {
  ...
  const apiStore = useApiStore()
  return apiStore.currentApiUrl || import.meta.env.VITE_APP_API_URL || 'http://localhost:8000'
}
