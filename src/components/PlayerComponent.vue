      ...
      <div class="iframe-wrapper" :style="!theaterMode ? iframeWrapperStyle : {}">
        <iframe
          ...
          :class="{
            ...
            'full-page-iframe': selectedPlayerInternal?.iframe?.includes('flcksbr.xyz') && !selectedPlayerInternal?.iframe?.includes('stloadi.live')
          }"
          @load="onIframeLoad"
          @error="onIframeError"
        ></iframe>
        <SpinnerLoading
          v-if="iframeLoading"
          :text="`Загружается плеер...\nЕсли плеер не грузится, попробуйте обновить страницу или включите VPN`"
...
const fetchPlayers = async () => {
  try {
    ...
    // Проверка на наличие kpId
    if (!props.kpId) {
      errorMessage.value = 'ID фильма не указан'
      errorCode.value = 400
    ...
    if (!players) {
      console.error('Плееры не найдены. Ответ от API:', players)
      errorMessage.value = 'Плееры не найдены'
      errorCode.value = 404
      iframeLoading.value = false
      return
    }
        ...
        try {
          ...
          while (searchPos > 0 && !found) {
            ...
            if (lastBracket > 0) {
              ...
              if (startPos < lastBracket) {
                const jsonCandidate = cleanString.substring(startPos, lastBracket + 1)
                try {
    ...
    // Всегда используем прямой URL на flcksbr.xyz
    // Это гарантирует, что будет использован только наш плеер
    const kpId = props.kpId.replace(/^(shiki)?/, '')
    const contentType = props.kpId.startsWith('shiki') ? 'series' : 'film'
    const flcksbrDirectUrl = `https://flcksbr.xyz/${contentType}/${kpId}`
    
    selectedPlayerInternal.value = {
      key: 'FLICKSBR',
      iframe: flcksbrDirectUrl,
      translate: 'Flcksbr',
      warning: false,
      name: 'flcksbr'
    }
    playersInternal.value = [selectedPlayerInternal.value]
    
    console.log('Используется прямой URL Flcksbr:', flcksbrDirectUrl)
    ...
    if (error.request) {
      ...
      errorMessage.value = 'Не удалось подключиться к серверу. Убедитесь, что бэкенд запущен на http://localhost:8000'
      errorCode.value = 503
    } else {
...
const onIframeLoad = () => {
  ...
  // Если используется прямой URL flcksbr.xyz, пытаемся скрыть лишние элементы
  if (selectedPlayerInternal.value?.iframe?.includes('flcksbr.xyz') && !selectedPlayerInternal.value?.iframe?.includes('stloadi.live')) {
    setTimeout(() => {
      try {
        const iframe = playerIframe.value
        const iframeDoc = iframe.contentDocument || iframe.contentWindow.document
        if (iframeDoc) {
          // Скрываем сайдбар, заголовки и другие лишние элементы
          const selectors = [
          ...
          // Ищем контейнер с плеером и делаем его на весь экран
          const playerSelectors = [
            '[class*="player"]', '[class*="video"]', '[id*="player"]', 
            '[id*="video"]', 'iframe.kinobox_iframe', '.kinobox_iframe'
          ]
          
          playerSelectors.forEach(selector => {
            try {
              const playerContainer = iframeDoc.querySelector(selector)
              if (playerContainer) {
                ...
                console.log('Настроен контейнер плеера:', selector)
              }
            } catch {
              // Игнорируем ошибки
            }
        ...
        console.log('Не удалось скрыть элементы (возможно, CORS ограничение):', error)
      }
    }, 500)
  }
...
const onIframeError = () => {
  console.error('Ошибка загрузки iframe:', selectedPlayerInternal.value?.iframe)
  
  // Если ошибка с stloadi.live, пробуем использовать прямой URL flcksbr.xyz
  if (selectedPlayerInternal.value?.iframe?.includes('stloadi.live') && props.kpId) {
    const kpId = props.kpId.replace(/^(shiki)?/, '')
    const contentType = props.kpId.startsWith('shiki') ? 'series' : 'film'
    const directUrl = `https://flcksbr.xyz/${contentType}/${kpId}`
    
    console.log('Ошибка загрузки stloadi.live, переключаемся на прямой URL:', directUrl)
    selectedPlayerInternal.value.iframe = directUrl
    iframeLoading.value = true
    // iframe автоматически перезагрузится с новым src
  } else {
    iframeLoading.value = false
    errorMessage.value = 'Не удалось загрузить плеер. Возможно, сервер недоступен или URL неверный.'
    errorCode.value = 404
  }
}
...
/* Скрываем лишние элементы при использовании прямого URL flcksbr.xyz */
/* Используем transform для обрезки видимой области */
/* Скрываем лишние элементы при использовании прямого URL flcksbr.xyz */
